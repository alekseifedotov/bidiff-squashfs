...
#B=$(HOME)/fw/coal-bump-core/ready-to-sign/bootloader/
B=$(HOME)/fw/coal-with-ppp/ready-to-sign/bootloader/
...
%.reconsitituted.img.checked: %.reconsitituted.img $(B)/%.img
	diff $^
	touch $@
...
clean:
	rm -f *.patch *.reconsitituted.img *.checked
